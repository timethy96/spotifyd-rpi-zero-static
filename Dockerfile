...
# Create a clean wrapper script for the cross-compiler
# We strip all architecture flags because Zig validates them strictly or defaults conflict.
# We align with Rust target arm-unknown-linux-musleabihf (Hard Float).
RUN echo '#!/bin/bash' > /usr/local/bin/arm-linux-musleabihf-gcc && \
    ...
    echo 'echo "WRAPPER GCC ARGS: $*" >&2' >> /usr/local/bin/arm-linux-musleabihf-gcc && \
...
RUN echo '#!/bin/bash' > /usr/local/bin/arm-linux-musleabihf-g++ && \
    ...
    echo 'echo "WRAPPER G++ ARGS: $*" >&2' >> /usr/local/bin/arm-linux-musleabihf-g++ && \
...
RUN echo '#!/bin/sh' > /usr/local/bin/arm-linux-musleabihf-ar && \
    echo 'exec zig ar "$@"' >> /usr/local/bin/arm-linux-musleabihf-ar && \
    chmod +x /usr/local/bin/arm-linux-musleabihf-ar

RUN echo '#!/bin/sh' > /usr/local/bin/arm-linux-musleabihf-ranlib && \
...
# 1. Download & Extract
RUN curl -fLO https://www.alsa-project.org/files/pub/lib/alsa-lib-1.2.10.tar.bz2 && \
...
# 1. Download & Extract - OpenSSL 1.1.1w (LTS, widely compatible)
RUN curl -fLO https://www.openssl.org/source/openssl-1.1.1w.tar.gz && \
    tar -xf openssl-1.1.1w.tar.gz

# 2. Configure
# We use linux-generic32 to avoid specific architecture assumptions that might conflict with our flags.
...
# Point to manually built OpenSSL
...
# Remove 'vendored' feature from Cargo.toml
RUN sed -i 's/^openssl = .*/openssl = "0.10"/' Cargo.toml

# Switch to bash for pipefail
SHELL ["/bin/bash", "-o", "pipefail", "-c"]

RUN find /build/sysroot -name "libssl.a"
RUN file /build/sysroot/usr/lib/libssl.a
